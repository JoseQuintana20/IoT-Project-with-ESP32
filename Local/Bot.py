...
#################################################################################################################################################################################################
# TOKEN
#bot = telebot.TeleBot("6423189133:AAHOlQX6GHWtoL4mfN8BZ-BTJpottNYsCNQ")
...
def send_welcome(message):
    if is_registered(message.chat.id):
        bot.reply_to(message,
                     "Hola, soy el Capybara Manolo. \n\n- Escribe /on para encender el Led. \n- Escribe /off para apagar el Led. \n- Escribe /estado para saber en qué estado está el Led.")
    else:
        bot.reply_to(message,
                     "Hola, soy el Capybara Manolo. \n\n Para usar el Bot, requiere registrarse. \n- Escribe /login para registrar")
...
def register_user(message):
    if is_registered(message.chat.id):
        ...
        bot.reply_to(message, "Por favor, ingresa la contraseña de acceso:")
...
def verify_password(message):
    ...
    if password == key:
        bot.reply_to(message, "Contraseña correcta. Por favor, ingresa tu correo para recuperar tu cuenta:")
...
def register_email(message):
    ...
    if re.match(r"[^@]+@[^@]+\.[^@]+", email):
        ...
        # Construir y ejecutar la consulta SQL
        query = "INSERT INTO public.login(chat_id, email) VALUES (%s, %s);"
        ...
        bot.reply_to(message, "Usuario registrado correctamente.")

        # Ejecutar el comando start/help después de registrar al usuario
        send_welcome(message)
    else:
        # Mensaje de error
        bot.reply_to(message, "Por favor, ingresa un correo electrónico válido.")
        bot.register_next_step_handler(message, register_email)
